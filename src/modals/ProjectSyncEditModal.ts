...
export class ProjectSyncEditModal extends Modal {
	...
	onOpen() {
		...
		new Setting(contentEl)
			...
			.addText((text) =>
				text
					.setPlaceholder('e.g. My Team Project')
		...
		new Setting(contentEl)
			...
			.addDropdown((drop) => {
				...
				drop.onChange((v) => {
					this.config.connectionIndex = parseInt(v);
				});
			});

		new Setting(contentEl)
			.setName('Project Key')
			.setDesc('Jira project key (all caps), e.g. PROJ')
			.addText((text) =>
				text
					.setPlaceholder('PROJ')
		...
		new Setting(contentEl)
			...
			.addText((text) =>
				text
					.setPlaceholder('jira-issues/PROJ')
					.setValue(this.config.targetFolder)
					.onChange((v) => {
						this.config.targetFolder = v;
					}),
			);
		...
		new Setting(contentEl)
			...
			.addToggle((t) =>
				t.setValue(this.config.deltaSync).onChange((v) => {
					this.config.deltaSync = v;
				}),
			);
		...
		new Setting(contentEl)
			...
			.addToggle((t) =>
				t.setValue(this.config.syncOnStartup).onChange((v) => {
					this.config.syncOnStartup = v;
				}),
			);
